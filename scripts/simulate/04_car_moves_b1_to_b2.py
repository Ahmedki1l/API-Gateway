#!/usr/bin/env python
"""Simulate a vehicle moving from a B1 slot to a B2 slot inside the garage.

The car never leaves the facility — VideoAnalytics simply observes:
  1. The old B1 slot becomes vacant -> unbind-slot on PMS-AI.
  2. The new B2 slot becomes occupied -> bind-slot on PMS-AI.

Optionally, with --with-line-events we also fire Hikvision linedetection
events from CAM-08 (B1 exit boundary) and CAM-09 (B2 entry boundary), which
shift zone_occupancy line-crossing counters.

Implementation strategy: shell out to the existing 03 (leave) and 02 (park)
scripts so all bind/unbind logic stays DRY in those leaf scripts.
"""
...
# Make _common.py importable when this script lives next to it.
...
CAM_08_IP = "10.1.13.67"  # B1 exit boundary line camera
CAM_09_IP = "10.1.13.68"  # B2 entry boundary line camera
...
def main() -> int:
    p = argparse.ArgumentParser(description="Simulate a B1 -> B2 in-garage slot change.")
    p.add_argument("--plate", default="TEST-A001")
    p.add_argument("--from-slot", default="B11_CFO",
                   help="Source slot on floor B1 (default: B11_CFO)")
    p.add_argument("--from-camera", default="CAM-03")
    p.add_argument("--to-slot", default="B16",
                   help="Destination slot on floor B2 (default: B16). "
                        "Per seed.sql, B2 floor slots include: "
                        "B14, B15, B16, B17, B18, B19, B20, B21, B22, B23, B24, B25, B27.")
    p.add_argument("--to-camera", default="CAM-09")
    ...
    cyan(f"=== B1 -> B2 move: plate={args.plate} "
    ...
    if args.with_line_events:
        cyan("[4/6] Posting linedetection events: CAM-08 (B1 exit) + CAM-09 (B2 entry)...")
        for cam_id, cam_ip in (("CAM-08", CAM_08_IP), ("CAM-09", CAM_09_IP)):
    ...
    green(f"  vehicle now in {args.to_slot} on floor B2")
    green("=== B1 -> B2 move PASS ===")
    print("PASS")
    return 0
